...
module Response = struct
  ...
  let equal = Keyval.Value.equal
  let to_json (t : t) : Json.t = `Assoc [ "value", `String (Keyval.Value.to_string t) ]
  ...
  let of_json (json : Json.t) : t =
    ...
    | `Assoc fields ->
      ...
      (match !value_ref with
       | Some s -> Keyval.Value.of_string s
...
let rpc : _ Rpc.t =
  Rpc.create
    ...
    ~version:1
    ~description:
      "Retrieve the value associated with a key from the memory database. Raises if the \
       key doesn't exist. Deprecated: use v2 instead, which returns null for a missing \
       key rather than failing the call."
