...
(* @mdexp ## Wrapping a long invocation *)

let%expect_test "multi-lines grouping highlight" =
  ...
  keyval
    ([ "this"; "that" ]
     :: List.init ~len:20 ~f:(fun i ->
       Printf.sprintf "--arg%i" i :: List.init ~len:i ~f:(fun j -> Int.to_string j)));
...
let%expect_test "margin used in pp" =
  (* @mdexp.code *)
