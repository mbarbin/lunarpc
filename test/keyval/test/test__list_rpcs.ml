...
let%expect_test "list all rpcs, compact" =
  ...
  let response =
    Rpc_client.call
      ...
      ~connection
      { include_schemas = false; names = All }
    |> Err.ok_exn
  in
  print_json (Rpc.List_rpcs.Response.to_json response);
...
let%expect_test "list a single rpc, with its schemas" =
  let@ t = Rpc_test_harness.run in
  let@ { server; client = _ } =
    Rpc_test_harness.with_server t ~config:Keyval_test.config
  in
  let@ connection = Rpc_test_harness.Server.with_connection server in
  (* @mdexp ## Filtered listing, with schemas
  ...
  let response =
    Rpc_client.call
      (module Rpc.List_rpcs)
      ~connection
      { include_schemas = true; names = Only { names = [ Rpc.Name.v "listKeys" ] } }
    |> Err.ok_exn
  in
  print_json (Rpc.List_rpcs.Response.to_json response);
  (* @mdexp.snapshot { lang: "json" } *)
